...
const ChatMessage: React.FC<ChatMessageProps> = ({ role, content, isStreaming }) => {
  ...
  return (
    <div className={`px-4 py-3 ${isUser ? '' : 'bg-gray-50 dark:bg-dark-bg'}`}>
      <div className="font-sans text-[14px] text-gray-800 dark:text-dark-text leading-relaxed whitespace-pre-wrap">
        {renderContent(content)}
        {isStreaming && !content && (
          <span className="inline-block w-1.5 h-4 bg-gray-400 dark:bg-dark-muted animate-pulse rounded-sm" />
        )}
      </div>
    </div>
  );
